...
async function fetchCharactersBySearch({ searchTerm, includeTags, excludeTags, nsfw, sort, page=1 }) {
    ...
    includeTags = includeTags.filter(tag => tag && tag.length > 0);
    if (includeTags && includeTags.length > 0) {
        searchParams.tags = includeTags.join(',').slice(0, 100);
    }

    excludeTags = excludeTags || [];
    excludeTags = excludeTags.filter(tag => tag && tag.length > 0);
    if (excludeTags && excludeTags.length > 0) {
        searchParams.exclude_tags = excludeTags.join(',').slice(0, 100);
    }

    console.log('Search params:', searchParams);

    // Try POST request first (as per API docs)
    let searchResponse;
    try {
        searchResponse = await fetch(API_ENDPOINT_SEARCH, {
            method: 'POST',
            headers: {
        ...
        const url = `${API_ENDPOINT_SEARCH}?${new URLSearchParams(searchParams).toString()}`;
        console.log('Fallback GET URL:', url);
        searchResponse = await fetch(url);
    }    if (!searchResponse.ok) {
        console.error(`API request failed with status ${searchResponse.status}: ${searchResponse.statusText}`);
    ...
    try {
        ...
        console.log('API response structure:', searchData);
    } catch (error) {
    ...
    if (searchData && searchData.data && searchData.data.nodes && Array.isArray(searchData.data.nodes)) {
        ...
        nodes = searchData;
    } else if (searchData && searchData.results && Array.isArray(searchData.results)) {
        nodes = searchData.results;
    } else if (searchData && searchData.data && Array.isArray(searchData.data)) {
        nodes = searchData.data;
    } else {
    ...
    if (nodes.length === 0) {
        ...
        return chubCharacters;
    }    console.log(`Found ${nodes.length} characters, processing...`);
    
    // Process characters with error handling
    const characterPromises = nodes.map(async (node, index) => {
        try {
            const fullPath = node.fullPath || node.path || node.id;
            if (!fullPath) {
                console.warn(`Character at index ${index} has no valid path:`, node);
                return null;
            }
            return await getCharacter(fullPath);
        } catch (error) {
            console.error(`Failed to fetch character at index ${index}:`, error);
            return null;
        }
    });
    ...
    characterBlobs.forEach((character, i) => {
        if (character && nodes[i]) {
            try {
                let imageUrl = URL.createObjectURL(character);
                const node = nodes[i];
                chubCharacters.push({
                    url: imageUrl,
                    description: node.tagline || node.description || "Description here...",
                    name: node.name || "Unnamed Character",
                    fullPath: node.fullPath || node.path || node.id,
                    tags: Array.isArray(node.topics) ? node.topics : (Array.isArray(node.tags) ? node.tags : []),
                    author: (node.fullPath || node.path || '').split('/')[0] || node.author || "Unknown",
                });
            } catch (error) {
                console.error(`Failed to process character ${i}:`, error);
            }
        }
    });
